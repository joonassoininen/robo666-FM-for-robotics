...
try:
    ...
    #------------------ Added to make the evaluation work---------------------
    eval_dir = Path("outputs/eval")
    eval_dir.mkdir(parents=True, exist_ok=True)
    eval_path = eval_dir / evaluation_file

    # evaluator = Evaluator(output_file=args.path)
    evaluator = Evaluator(output_file=eval_path)
    ...
    # Custom recording_manager config to get rid of the errors
    ...
    img1 = obs1["observation.images.table"]
    img2 = obs2["observation.images.table"]
    
    # Debugging: 
    
    #print("table img shape:", np.array(img1).shape, np.array(img2).shape)
    #print("table img mean:", np.array(img1).mean(), np.array(img2).mean())
    #print("table img abs diff mean:", np.mean(np.abs(np.array(img2, dtype=np.float32) - np.array(img1, dtype=np.float32))))

    hand1 = obs1["observation.images.hand"]
    hand2 = obs2["observation.images.hand"]
    
    # Debugging: 
    
    #print("hand img abs diff mean:", np.mean(np.abs(np.array(hand2, dtype=np.float32) - np.array(hand1, dtype=np.float32))))

    logger.info(f"OBS KEYS: {sorted(obs.keys())}")

    logger.info(f"STATE-LIKE KEYS: {[k for k in obs.keys() if 'state' in k]}")
        ...
        with recording_manager:
            while not recording_manager.done():
                ...
                recording_manager.record_episode(
                    ...
                    task="Pick up the green cube and place it on to the red square.",
                    on_start=on_start,
                    on_end=on_end,
                )
