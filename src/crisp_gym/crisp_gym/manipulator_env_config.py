...
class RightNoCamFrankaEnvConfig(NoCamFrankaEnvConfig):
    ...
    gripper_config: GripperConfig | None = field(
        default_factory=lambda: GripperConfig.from_yaml(
            path=(BASE_DIR / "config" / "panda_gripper.yaml").resolve()
        )
    )
    ...
    camera_configs: List[CameraConfig] = field(
        default_factory=lambda: [
            CameraConfig(
                camera_name="table",
                camera_frame="table_camera",   
                resolution=(256, 256),               
                camera_color_image_topic="table_camera/image_raw",
                camera_color_info_topic="table_camera/camera_info",
            ),
            CameraConfig(
                camera_name="hand",
                camera_frame="panda_hand_camera_optical",
                resolution=(256, 256),
                camera_color_image_topic="hand_camera/image_raw",
                camera_color_info_topic="hand_camera/camera_info",
            ),
        ]
    )
