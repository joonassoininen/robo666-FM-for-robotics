...
class ManipulatorBaseEnv(gym.Env):
    ...
    def __init__(self, config: ManipulatorEnvConfig, namespace: str = ""):
        ...
        # ---------- Lisätty ---------------
        self._gripper_effort_pub = None
        if self.config.gripper_enabled:
            # namespace huomio: jos namespace="right", topic on "/right/gripper_effort_controller/commands"
            topic = f"{('/' + namespace) if namespace else ''}/gripper_effort_controller/commands"
            self._gripper_effort_pub = self.robot.node.create_publisher(Float64MultiArray, topic, 10)
            logger.info(f"Gripper effort pub -> {topic}")
    ...
    def initialize(self, force: bool = False):
        ...
        if self.config.joint_control_param_config:
            ...
            #        self.config.joint_controller_parameters,  # or whatever your config field is
            #        ignore_missing_params=True,               # if present in your version
            #    )
    ...
    def _set_gripper_action(self, action: float):
        ...
        # map intent -> effort command
        # HUOM: sinun teleop-havaintosi mukaan:
        # open  -> [-15, -15]
        # close -> [  7,   7]
        if a > 0.3:
            cmd = [7.0, 7.0]   # open
        ...
        if self._gripper_effort_pub is None:
            logger.warning("Gripper effort publisher not initialized.")
            return

        msg = Float64MultiArray()
        msg.data = cmd
        self._gripper_effort_pub.publish(msg)
