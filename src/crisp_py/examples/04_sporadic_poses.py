...
from crisp_py.robot_config import MyRobotConfig
from crisp_py.utils.geometry import Pose as BasePose
...
def _rotation(self):
    if hasattr(self, "orientation"):
        ...
        return np.eye(3)
BasePose.rotation = property(_rotation)

#robot = Robot(namespace="")  # "right"
#robot = Robot()
my_robot_config = MyRobotConfig()
robot = Robot(robot_config=my_robot_config, namespace="")
...
delta_pose = Pose(
    position=np.array([0.3, 0.3, 1.2]),
    orientation=Rotation.from_euler("xyz", [np.pi, 0.0, 0.0], degrees=False),
)
...
print(type(robot.end_effector_pose))
...
def get_error(target_poses, ee_poses):
    ...
    d_rot = np.array([
    Rotation.from_matrix(R_diff).as_rotvec()
    for R_diff in (R_ee @ R_t.transpose(0, 2, 1))
    ])
    drx = d_rot[:, 0]
    dry = d_rot[:, 1]
    drz = d_rot[:, 2]
